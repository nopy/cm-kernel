...
static long m4mo_set_sensor_mode(int mode)
{
	int i=0;
	unsigned short read_value_1=0;

	switch (mode) {
	case SENSOR_PREVIEW_MODE_OLD:
...
int m4mo_sensor_config(void __user *argp)
{
	...
	case CFG_SET_MODE_OLD:
		rc = m4mo_set_sensor_mode(
					cfg_data.mode);
		break;

	case CFG_SET_EFFECT_OLD:
		rc = m4mo_set_effect(
					cfg_data.mode,
					cfg_data.cfg.effect);
		break;
...
static int cam_pm_lp8720_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("cam_pm_lp8720_probe: enter\n");
	PGH_DEBUG("START......\n");
...
static int __m4mo_probe(struct platform_device *pdev)
{
	int32_t rc = 0;
	...
	return msm_camera_drv_start(pdev, m4mo_probe_init);
...
static int __init m4mo_module_init(void)
{
	return platform_driver_register(&msm_camera_driver);
}

module_init(m4mo_module_init);
